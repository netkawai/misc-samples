...
inline wchar32_t DecodeUtf8( const unsigned char* pSrc, const int nSrcLen )
{
	...
	/*!
		Determine special conversion for WinAPI function WideCharToMultiByte
		Unicode characters for which this function returns true are
		@code
		::WideCharToMultiByte( 932, 0, src, srclen, dest, destlen, NULL, &bUsedDefaultChar );
		@endcode
		characters that pass the conversion process in SJIS (CP 932) but do not convert properly to SJIS (CP 932).
		It seems that they want you to avoid using the WC_NO_BEST_FIT_CHARS flag that can be used from Win98. \n
