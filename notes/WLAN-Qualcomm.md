...
Upstreaming Support for Qualcomm PCie Modems
old.linaro.org
7 min
June 1, 2021
View Original
...
As any other bus, MHI devices (controllers, clients) are represented under sysfs hierarchy: $ ls /sys/bus/mhi/devices mhi0 mhi0_DIAG mhi0_IP_HW0 mhi0_QMI
...
After some LKML back and forth to refine things, we migrated to a better solution by splitting our work into two parts. The first of these was a generic new WWAN subsystem, which is a hardware agnostic framework managing and exposing WWAN devices and their control ports. The second is a MHI specific WWAN port driver, mhi_wwan_ctrl, which registers to the newly created WWAN subsystem and performs the MHI bus adaptation. This way, the WWAN framework can be used with any further WWAN drivers.
WWAN made easy
...
Enjoy!
For more information on Linaro and the work that we do, contact us here.
Pocket - Upstreaming Support for Qualcomm PCie Modems
